...
client.on('ready', async () => {
    ...
    try {
        ...
        // Filtrar los chats para solo obtener los grupos
        ...
        if (groups.length > 0) {
            ...
            console.log('🔹 No se encontraron grupos en el chat.');
        }
    } catch (error) {
...
client.on('message', async message => {
    ...
    if (keywords.some(word => message.body.toLowerCase().includes(word.toLowerCase()))) {
        ...
        if (chat.isGroup) {
            ...
            const grupoDestinoId = 'ID_DEL_GRUPO_DESTINO'; // Reemplaza con el ID real del grupo destino
