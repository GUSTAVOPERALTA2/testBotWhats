...
client.on('message', async message => {
    ...
    const groupAmaId = '120363409776076000@g.us'; // ID del grupo Ama (nuevo)
    ...
    // Verificar si alguna palabra está en el Set de keywords AMA
    if (words.some(word => keywordsAma.has(word))) {
        foundAma = true;
    }
    ...
    if (foundAma) {
        console.log('🔹 Mensaje contiene una palabra clave AMA, reenviando al grupo Ama...');
        const targetChatAma = await client.getChatById(groupAmaId);
        await targetChatAma.sendMessage(message.body);
        console.log('✅ Mensaje reenviado al grupo "Ama".');
    }
});
